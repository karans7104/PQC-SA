...
\caption{Gantt chart comparison: Segatz schedule (left) vs.\ HLFET-optimal schedule (right) under Scenario~A (PC baseline) for all three KEM operations. Both schedules achieve identical makespans, confirming the 0.0\% gap under software-only timing.}
...
% Placeholder for DAG figure — to be provided
% \begin{figure}[!t]
% \centering
% \includegraphics[width=\columnwidth]{../sim/results/dag_keygen.png}
% \caption{Key generation DAG with critical path highlighted (bold edges). Tasks KG.1 through KG.9 with precedence edges and measured weights. The critical path KG.1 $\rightarrow$ KG.2 $\rightarrow$ KG.7 $\rightarrow$ KG.8 $\rightarrow$ KG.9 consumes 87.5\% of total work.}
% \label{fig:dag_keygen}
% \end{figure}
